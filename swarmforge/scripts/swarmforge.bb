...
(defn validate-window! [ctx line-no role agent worktree receive-mode roles worktrees]
  ...
  (reject-if (not (#{"claude" "codex" "copilot" "grok" "opencode"} agent))
...
(defn write-tmux-env-file! [ctx]
  (spit (str (:tmux-env-file ctx))
        (str (sh-out "tmux" "-S" (:tmux-socket ctx) "display-message" "-p"
                     "-t" (:session (first (:roles ctx)))
  ...
  (let [role (:role row)
    ...
    (cond-> (str base
                (case agent
                  ...
                  "opencode" (str "opencode run --dangerously-skip-permissions \"$(cat " (sq (str prompt-file)) ")\"")))
      (and (= index 0) (not= "1" (System/getenv "SWARMFORGE_KEEP_SESSIONS")))
